...
def test_stability_uses_seed_order_not_runner_completion_order() -> None:
    # Rows deliberately arrive in reverse seed order, as the Rust runner may
    # return completed games in completion order.  The cumulative stability
    # curve must still follow seed order.
    rows = [
        row("draw", point=0, seed=1),
        row("draw", point=100, seed=2),
        row("draw", point=0, seed=3),
        row("draw", point=100, seed=4),
    ]
    candidate = _summarize(list(reversed(rows)), 0, "1s")
    assert candidate["stability"] == [
        {"games": 1, "average_point": 0.0},
        {"games": 2, "average_point": 50.0},
        {"games": 3, "average_point": 100.0 / 3},
        {"games": 4, "average_point": 50.0},
    ]
