    ...
    def forward(self, obs: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        if self.amp_autocast:
            ...
            with torch.autocast("cuda", enabled=True):
                phi = self.brain(obs)
                if self.output_kind == "advantage":
                    _, advantage = self.dqn.net(phi).split((1, 46), dim=-1)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        choices=("q", "advantage"),
        default="q",
        help="formal Lite uses raw advantage; q is retained for legacy parity probes",
    )
...
'''
            ...
            if isinstance(paths, list):
                ...
                for path in paths:
                    if path.endswith(".wrapper.pyd"):
                        ...
                        # Preserve the libtorch-free AOTI model code as a
                        # standalone PE DLL for MortalSim's tiny native host.
                        args.output_dir.mkdir(parents=True, exist_ok=True)
                        shutil.copyfile(path, args.output_dir / "model.dll")
                return rewritten
            return paths
    ...
    if (version, channels, blocks) != (4, 256, 54):
        raise ValueError(
            "Formal Lite supports only Mortal v4 with 256 channels and 54 residual blocks; "
            f"got v{version}/{channels}/{blocks}"
        )
    ...
    args.output_dir.mkdir(parents=True, exist_ok=True)
    suffix = "amp" if amp_static else ("autocast" if amp_autocast else "fp32")
    if args.output_kind == "advantage":
        suffix += "-advantage"
    ...
    manifest = {
        ...
        "decision_contract": (
            "stable_advantage_v2" if args.output_kind == "advantage" else "legacy_amp_v1"
        ),
        "precision_profile": f"{args.precision}-{args.output_kind}",
        "batch_capacity": args.batch,
        "compute_capability": ".".join(map(str, torch.cuda.get_device_capability(0))),
        "architecture": {"version": version, "conv_channels": channels, "num_blocks": blocks},
        "checkpoint_sha256": sha256(args.checkpoint),
        "graph": {"file": pte_path.name, "sha256": sha256(pte_path)},
        "tensor_data": [
            {"file": path.name, "sha256": sha256(path)}
            for path in sorted(args.output_dir.glob("*.ptd"))
        ],
        "toolchain": {
            "python": sys.version.split()[0],
            "pytorch": torch.__version__,
            "executorch": installed_version("executorch"),
            "triton_windows": installed_version("triton-windows"),
            "cuda": torch.version.cuda,
            "host_compiler": platform.python_compiler(),
        },
    }
