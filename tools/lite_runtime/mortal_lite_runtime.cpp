...
__declspec(dllexport) int mortal_lite_device_compute_capability(
    int device,
    int* major,
    int* minor,
    char* error,
    size_t error_size) {
  if (major == nullptr || minor == nullptr || device < 0) {
    set_error(error, error_size, "invalid CUDA capability query arguments");
    return 1;
  }
...
__declspec(dllexport) const char* mortal_lite_runtime_abi() {
  return "mortalsim-lite-abi-2";
}
...
__declspec(dllexport) void* mortal_lite_create(
  ...
  runtime->host_mask.resize(mask_count, 0);
...
__declspec(dllexport) int mortal_lite_run(
  ...
  // The formal graph always runs at capacity. Padded rows are semantically
  // absent, so both their observations and legal-action masks must be zero.
  std::fill(runtime->host_mask.begin() + actual_mask, runtime->host_mask.end(), 0);
  if (!check_cuda(
          cudaMemcpyAsync(
              runtime->device_obs,
