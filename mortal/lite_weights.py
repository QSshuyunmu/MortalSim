...
def _read_checkpoint(path: Path) -> tuple[dict[str, Any], dict[str, bytes]]:
    try:
        if path.stat().st_size > MAX_CHECKPOINT_BYTES:
            raise LiteWeightError("checkpoint exceeds the 2 GiB limit")
    except OSError as exc:
        raise LiteWeightError(f"checkpoint cannot be read: {exc}") from exc
    try:
    ...
    with archive:
        ...
        if len(infos) > MAX_ARCHIVE_ENTRIES:
            raise LiteWeightError("checkpoint ZIP contains too many entries")
        if any(info.compress_type != zipfile.ZIP_STORED for info in infos):
            raise LiteWeightError("compressed checkpoint entries are unsupported")
        if sum(info.file_size for info in infos) > MAX_CHECKPOINT_BYTES:
            raise LiteWeightError("checkpoint uncompressed data exceeds the 2 GiB limit")
        ...
        if pickle_info.file_size > MAX_PICKLE_BYTES:
            raise LiteWeightError("checkpoint metadata is too large")
