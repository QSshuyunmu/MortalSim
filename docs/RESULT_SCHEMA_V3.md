...
## Hanchan rank extension (metrics v3)

`metrics_version: 3` adds `candidate.hanchan` and a top-level
`hanchan_model` identity block.  See `HANCHAN_RANK_MODEL.md` for the field
layout and `HANCHAN_PT_TABLE.md` for the Houou dan pt tables.

Extension merge requires identical `hanchan_model.model_id`,
`hanchan_model.model_sha256`, and `hanchan_model.feature_schema` in addition
to the existing runtime/model identity checks.
