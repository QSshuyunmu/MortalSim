...
在同一固定 300 局、14,397 次决策轨迹上又比较了三个已经可以运行的原生图：

| 图 | 精度/卷积实现 | 动作分歧 | 分歧位置 |
|---|---|---:|---|
| AMP-static GEMM | FP16 参数、BN FP32、GEMM 展开 Conv1d | 3 | 11519、12420、13701 |
| AMP-static Native Conv | 同上、原生 Conv1d | 3 | 4806、12420、13701 |
| FP32 | 全 FP32 | 6 | 616、4806、6221、7985、11519、11937 |

两个 AMP 图在 12420、13701 处同时产生相同的近似并选择了错误动作，而 Native
Conv 在 4806、GEMM 在 11519 处分别出现不同的近似；FP32 图虽然修正了前述两处，
却在其它状态引入新的分歧。三者没有稳定的多数投票规则，不能用动作偏置、索引
优先级或观察值哈希表“修正”而宣称严格一致。正式 Lite 因此继续保留 PyTorch AMP
Reference 作为权威路径，原生图只能作为实验性压缩运行时。
