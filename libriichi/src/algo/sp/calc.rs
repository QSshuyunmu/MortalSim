...
type StateCache = [AHashMap<State, usize>; SHANTEN_THRES as usize + 1];

#[derive(Default)]
struct SPWorkspaceFor<const MAX_TSUMO: usize> {
    discard_cache: StateCache,
    draw_cache: StateCache,
    values: Vec<Values<MAX_TSUMO>>,
}

impl<const MAX_TSUMO: usize> SPWorkspaceFor<MAX_TSUMO> {
    fn clear(&mut self) {
        for cache in self
            .discard_cache
            .iter_mut()
            .chain(self.draw_cache.iter_mut())
        {
            cache.clear();
        }
        self.values.clear();
...
/// Reusable allocation storage for repeated single-player calculations.
///
/// The calculation caches are invalid across calls, so each call clears their
/// contents while retaining the hash table allocations.
#[derive(Default)]
pub(crate) struct SPWorkspace {
    n1: Option<SPWorkspaceFor<1>>,
    n2: Option<SPWorkspaceFor<2>>,
    n3: Option<SPWorkspaceFor<3>>,
    n4: Option<SPWorkspaceFor<4>>,
    n5: Option<SPWorkspaceFor<5>>,
    n6: Option<SPWorkspaceFor<6>>,
    n7: Option<SPWorkspaceFor<7>>,
    n8: Option<SPWorkspaceFor<8>>,
    n9: Option<SPWorkspaceFor<9>>,
    n10: Option<SPWorkspaceFor<10>>,
    n11: Option<SPWorkspaceFor<11>>,
    n12: Option<SPWorkspaceFor<12>>,
    n13: Option<SPWorkspaceFor<13>>,
    n14: Option<SPWorkspaceFor<14>>,
    n15: Option<SPWorkspaceFor<15>>,
    n16: Option<SPWorkspaceFor<16>>,
    n17: Option<SPWorkspaceFor<17>>,
    #[cfg(feature = "sp_reproduce_cpp_ver")]
    n18: Option<SPWorkspaceFor<18>>,
}
...
struct SPCalculatorState<'a, 'workspace, const MAX_TSUMO: usize> {
    ...
    discard_cache: &'workspace mut StateCache,
    draw_cache: &'workspace mut StateCache,
    values: &'workspace mut Vec<Values<MAX_TSUMO>>,
...
impl SPCalculator<'_> {
    ...
    pub fn calc(
        ...
        self.calc_with_workspace(
            init_state,
            can_discard,
            tsumos_left,
            cur_shanten,
            &mut SPWorkspace::default(),
        )
    }

    pub(crate) fn calc_with_workspace(
        &self,
        init_state: InitState,
        can_discard: bool,
        tsumos_left: u8,
            ...
            ($(($n:literal, $field:ident)),*) => {
                match max_tsumo {
                    $($n => {
                        ...
                        let workspace = workspace.$field.get_or_insert_default();
                        workspace.clear();
                        let mut calc_state = SPCalculatorState::<$n> {
                            ...
                            discard_cache: &mut workspace.discard_cache,
                            draw_cache: &mut workspace.draw_cache,
                            values: &mut workspace.values,
                            #[cfg(feature = "sp_reproduce_cpp_ver")]
        ...
        #[cfg(feature = "sp_reproduce_cpp_ver")]
        let candidates = static_expand!((17, n17), (18, n18));
        #[cfg(not(feature = "sp_reproduce_cpp_ver"))]
...
impl<const MAX_TSUMO: usize> SPCalculatorState<'_, '_, MAX_TSUMO> {
    ...
    fn draw(&mut self, shanten: i8) -> usize {
    ...
    fn draw_with_tegawari(&mut self, shanten: i8) -> usize {
    ...
    fn draw_with_tegawari_slow(&mut self, shanten: i8) -> usize {
            ...
            for i in 0..MAX_TSUMO {
                ...
                match &scores_or_values {
                    ...
                    ScoresOrValues::Values(next_values) => {
                        let next_values = &self.values[*next_values];
                        if shanten == 1 {
                            // 1向聴の場合は次で聴牌
        ...
        } in draw_tiles
        {
            ...
            self.state.deal(tile);
            self.state.n_extra_tsumo += 1;
            let next_values_idx = self.discard(shanten);
            self.state.n_extra_tsumo -= 1;
            self.state.undo_deal(tile);
            let next_values = &self.values[next_values_idx];
        ...
        let values_idx = self.values.len();
    ...
    fn discard_slow(&mut self, shanten: i8) -> usize {
        ...
        for DiscardTile { tile, shanten_diff } in discard_tiles {
                ...
                && shanten < SHANTEN_THRES
            {
                ...
                self.state.discard(tile);
                self.state.n_extra_tsumo += 1;
                values_idx = self.draw(shanten + 1);
                self.state.n_extra_tsumo -= 1;
                self.state.undo_discard(tile);
            } else {
