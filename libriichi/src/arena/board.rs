...
impl Board {
    ...
    /// Like `into_state` but with an explicit `oya` instead of deriving it
    /// from `kyoku % 4`.  Used by `CustomKyokuRunner` where the caller
    /// specifies which seat is the dealer independently of the round number.
    pub fn into_state_with_oya(self, oya: u8) -> BoardState {
        let dora_indicators_full = self.dora_indicators.clone();
...
#[rustfmt::skip]
pub(crate) const UNSHUFFLED: [Tile; 136] = [
    t!(1m),  t!(1m), t!(1m), t!(1m),
    t!(2m),  t!(2m), t!(2m), t!(2m),
    t!(3m),  t!(3m), t!(3m), t!(3m),
    t!(4m),  t!(4m), t!(4m), t!(4m),
