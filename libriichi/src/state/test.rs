    ...
    fn validate(&self) {
        ...
        if self.last_cans.can_act() {
            for version in 1..=MAX_VERSION {
                self.assert_encode_into_matches(version, false);
                if self.last_cans.can_kakan || self.last_cans.can_ankan {
                    self.assert_encode_into_matches(version, true);
                }
            }
        }
    }

    fn assert_encode_into_matches(&self, version: u32, at_kan_select: bool) {
        let (obs, mask) = self.encode_obs(version, at_kan_select);
        let mut actual_obs = vec![f32::NAN; obs.len()];
        let mut actual_mask = vec![true; mask.len()];
        self.encode_obs_into(version, at_kan_select, &mut actual_obs, &mut actual_mask);
        assert_eq!(obs.as_slice().unwrap(), actual_obs);
        assert_eq!(mask.as_slice().unwrap(), actual_mask);

        let mut workspace = SPWorkspace::default();
        for _ in 0..2 {
            actual_obs.fill(f32::NAN);
            actual_mask.fill(true);
            self.encode_obs_into_with_workspace(
                version,
                at_kan_select,
                &mut actual_obs,
                &mut actual_mask,
