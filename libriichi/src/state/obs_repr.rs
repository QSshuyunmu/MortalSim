...
use crate::algo::sp::{Candidate, CandidateColumn, SPWorkspace};
use crate::array::Simple2DArrayView;
...
struct ObsEncoderContext<'state, 'buffer> {
    state: &'state PlayerState,
    arr: Simple2DArrayView<'buffer, 34, f32>,
    mask: &'buffer mut [bool],
    idx: usize,
    at_kan_select: bool,
    version: u32,
    sp_workspace: Option<&'buffer mut SPWorkspace>,
}
...
impl IntegerEncoder {
    ...
    fn encode(self, ctx: &mut ObsEncoderContext<'_, '_>) {
...
impl<'state, 'buffer> ObsEncoderContext<'state, 'buffer> {
    fn new(
        state: &'state PlayerState,
        version: u32,
        at_kan_select: bool,
        ...
        Self::new_with_workspace(state, version, at_kan_select, obs, mask, None)
    }
    ...
    fn new_with_workspace(
        ...
        assert_eq!(obs.len(), shape.0 * shape.1);
        assert_eq!(mask.len(), ACTION_SPACE);
        obs.fill(0.);
        mask.fill(false);
        let arr = Simple2DArrayView::new(obs);
        Self {
            state,
            arr,
            mask,
    ...
    fn encode_obs(mut self) {
        ...
        if self.version == 4 {
            let tables = if let Some(workspace) = self.sp_workspace.as_deref_mut() {
                state.single_player_tables_with_workspace(workspace)
            } else {
                state.single_player_tables()
            };
            if let Ok(SinglePlayerTables { max_ev_table }) = tables {
                ...
                if cans.can_discard {
                    ...
                    if let Some(max_required) = max_ev_table
                        .iter()
                        .max_by(|l, r| l.cmp(r, CandidateColumn::NotShantenDown))
                    {
                        let max_required_tiles_tid = max_required.tile.deaka().as_usize();
...
impl PlayerState {
    ...
    pub fn encode_obs(&self, version: u32, at_kan_select: bool) -> (Array2<f32>, Array1<bool>) {
        let shape = obs_shape(version);
        let mut obs = Array2::zeros(shape);
        let mut mask = Array1::default(ACTION_SPACE);
        self.encode_obs_into(
            version,
            at_kan_select,
            obs.as_slice_mut().unwrap(),
            mask.as_slice_mut().unwrap(),
        );
        (obs, mask)
    }

    pub(crate) fn encode_obs_into(
        &self,
        version: u32,
        at_kan_select: bool,
        obs: &mut [f32],
        ...
        ObsEncoderContext::new(self, version, at_kan_select, obs, mask).encode_obs();
    }

    pub(crate) fn encode_obs_into_with_workspace(
        &self,
        version: u32,
        at_kan_select: bool,
        obs: &mut [f32],
        ...
        ObsEncoderContext::new_with_workspace(
            self,
            version,
            at_kan_select,
            obs,
