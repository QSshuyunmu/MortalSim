...
pub struct Simple2DArrayView<'a, const COLS: usize, T> {
    arr: &'a mut [T],
}

impl<'a, const COLS: usize, T> Simple2DArrayView<'a, COLS, T>
where
...
mod test {
    ...
    #[test]
    fn borrowed_storage_matches_owned_storage() {
        let mut storage = vec![0; 8];
        let mut arr = Simple2DArrayView::<2, i32>::new(&mut storage);
        arr.fill(1, 3);
        arr.assign_rows(2, 0, 2, 7);

        assert_eq!(storage, vec![0, 0, 3, 3, 7, 0, 7, 0]);
    }
}
