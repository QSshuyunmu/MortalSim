...
if hanchan_model_dir.exists():
    datas.append((str(hanchan_model_dir), "models/hanchan_rank"))
for source in (ROOT / "mortal").glob("*.py"):
    datas.append((str(source), "mortal"))

# LightGBM is a native-code package: collect its data, DLLs and submodules
# explicitly for the libtorch-free build.
...
hiddenimports = list(collect_submodules("openpyxl")) + _lgb_hidden + [
    "h11",
...
excluded = [
    ...
    "pandas", "pyarrow", "matplotlib", "numba", "llvmlite", "sklearn",
