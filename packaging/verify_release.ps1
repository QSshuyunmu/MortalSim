...
$checksumFiles = @(
    ...
    (Join-Path $release 'SHA256SUMS-Lite.txt')
...
$archives = @(Get-ChildItem -LiteralPath $release -Filter 'MortalSim-Windows-x64-*-v*.zip' -File)
...
try {
    ...
    $requiredFiles = if ($lite.Count -eq $archives.Count) {
        @('RELEASE_MANIFEST.json', 'SBOM.cdx.json', 'Start-MortalSim.cmd', 'Install-MortalSim.ps1', 'legal/LICENSE', 'legal/NOTICE', 'legal/THIRD_PARTY_LICENSES.md', 'legal/MODEL_LICENSE.md')
    } else {
