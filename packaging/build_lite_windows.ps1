...
# Remove pre-v0.3 aliases whose filename is not part of the signed manifest.
Get-ChildItem -LiteralPath $runtime -Filter "*.wrapper.pyd" -File -ErrorAction SilentlyContinue | Remove-Item -Force
foreach ($name in $required) {
    Copy-Item -LiteralPath (Join-Path $ArtifactDir $name) -Destination (Join-Path $runtime $name) -Force
}
$pythonVersion = (Get-Content pyproject.toml | Select-String '^version =').ToString().Split('"')[1]
$version = if ($pythonVersion -match '^(\d+\.\d+\.\d+)rc(\d+)$') { "$($Matches[1])-rc.$($Matches[2])" } else { $pythonVersion }
$stage = Join-Path $root "build/lite-stage"
$releaseDir = if ([IO.Path]::IsPathRooted($ReleaseDirectory)) { $ReleaseDirectory } else { Join-Path $root $ReleaseDirectory }
...
foreach ($file in @("INSTALL.md", "USER_GUIDE.md", "TROUBLESHOOTING.md", "SMOKE_TEST.md", "LITE_VALIDATION.md", "PARITY_ROOT_CAUSE.md", "DECISION_CONTRACTS.md", "RESULT_SCHEMA_V3.md")) {
...
@{
    ...
    variant = "Lite"
    version = $version
    python_package_version = $pythonVersion
    model_included = $false
    decision_contract = "stable_advantage_v2"
    runtime = "AOTInductor CUDA SM89; import a compatible v4 .pth in the app"
    files = @($required)
...
Copy-Item (Join-Path $stage "SBOM.cdx.json") (Join-Path $releaseDir "SBOM.cdx.json") -Force
Copy-Item (Join-Path $root "docs\LITE_VALIDATION-v0.3.0-rc.1.json") (Join-Path $releaseDir "LITE_VALIDATION-v0.3.0-rc.1.json") -Force
