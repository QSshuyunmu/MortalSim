...
type RunResult = { schema_version?: number; metrics_version?: number; run_id?: string; elapsed?: number; runs?: number; total_runs?: number; seed?: number; candidates?: Candidate[]; summaries?: Candidate[]; comparisons?: any[]; warnings?: string[]; config?: Record<string, any>; resolved_context?: Record<string, any>; resolved_input?: { main_haipai?: string[]; first_tsumo?: string; dora?: string }; extension_history?: any[]; gpu_telemetry?: Record<string, any>; model?: ModelInfo };
type RunRecord = { run_id: string; status: string; created_at: string; request: Record<string, any>; result?: RunResult };
type ModelInfo = { id:string; label:string; filename:string; sha256?:string; version?:number; conv_channels?:number; num_blocks?:number; ready:boolean; error?:string; source:string };
...
const initial = {
  hand:"4567m3477p134066s", dora:"9s", discards:"1s, 6s",
  riichi_discards:[] as string[],
  round:"E1", honba:"0", kyotaku:"0",
  scores:{self:"25000",shimocha:"25000",toimen:"25000"},
  runs:"1000", seed:"42", batch_size:"1000", rayon_threads:"20", model_id:"mortal-v4-20240308", strict_comparison:true,
};
...
export default function App() {
  ...
  const refresh=()=>fetch("/api/runs").then(r=>r.json()).then(setRuns).catch(()=>{});
  const refreshModels=()=>fetch("/api/models").then(r=>r.json()).then(setModels).catch(()=>{});
  const refreshTasks=()=>fetch("/api/tasks/active").then(r=>r.json()).then(setTasks).catch(()=>{});
  useEffect(()=>{fetch("/api/capabilities").then(r=>r.json()).then(setCap).catch(()=>setCap({cuda_available:false}));refresh();refreshModels();refreshTasks();},[]);
  useEffect(()=>{
    const selected=models.find(model=>model.id===config.model_id);
    const firstReady=models.find(model=>model.ready);
    if((!selected||!selected.ready)&&firstReady) setConfig(old=>({...old,model_id:firstReady.id}));
  ...
  return <div className="shell">
    ...
    <main><header><div><span className="kicker">LOCAL RIICHI SIMULATION</span><h1>{page==="run"?"自亲第一打分析":page==="history"?"历史运行":"设置与诊断"}</h1></div><div className="header-status"><button className={`task-toggle ${tasks.length?"active":""}`} onClick={()=>setTaskDrawer(true)} aria-label="运行任务"><Layers3 size={15}/>任务 {tasks.length||""}</button><Activity size={15}/><span>数据仅保留在本机</span><code>v0.2.0-alpha.0</code></div></header>
      ...
      {page==="run"&&<>{!result&&<Workbench config={config} update={update} updateScore={updateScore} candidates={candidates} start={start} cancel={cancel} busy={busy} status={status} progress={progress} cap={cap} gpu={gpu} models={models} refreshModels={refreshModels}/>} {result&&<Results result={result} initialExtend={pendingExtend} onExtendOpened={()=>setPendingExtend(false)} onCreated={()=>{setPendingExtend(false);refreshTasks();setNotice("扩容已在后台开始，可继续浏览其他记录。");}} onNew={()=>{setResult(null);setStatus("idle");}}/>}</>}
      {page==="history"&&<History runs={runs} activeTasks={tasks} open={openRun} extend={run=>{openRun(run);setPendingExtend(true);}} refresh={refresh}/>}
...
function Workbench({config,update,updateScore,candidates,start,cancel,busy,status,progress,cap,gpu,models,refreshModels}:any){
  ...
  const modelReady=models.some((model:ModelInfo)=>model.id===config.model_id&&model.ready);
...
function ModelPicker({models,modelId,change,refresh}:any){
  ...
  const upload=async(file:File)=>{setMessage(`正在校验 ${file.name}…`);const response=await fetch(`/api/models/import?filename=${encodeURIComponent(file.name)}`,{method:"POST",headers:{"Content-Type":"application/octet-stream"},body:file});if(!response.ok){const body=await response.json().catch(()=>({}));setMessage(apiErrorMessage(body.detail,"模型导入失败"));return;}const model=await response.json();refresh();change(model.id);setMessage(model.duplicate?"模型已在本机库中，已选中。":"模型已通过 CUDA 自检并可使用。");};
  const selected=models.find((model:ModelInfo)=>model.id===modelId);
  return <div className="model-picker"><label className="wide"><span>推理模型</span><select value={modelId} onChange={event=>change(event.target.value)}>{models.map((model:ModelInfo)=><option value={model.id} key={model.id} disabled={!model.ready}>{model.label} · v{model.version||"?"} · {model.ready?"可用":"不可用"}</option>)}</select></label><button type="button" className="secondary import-model" onClick={()=>inputRef.current?.click()}><FileUp size={15}/>导入 .pth</button><input ref={inputRef} className="visually-hidden" type="file" accept=".pth" onChange={event=>{const file=event.target.files?.[0];if(file)upload(file);event.currentTarget.value="";}}/>{selected?.ready?<p className="model-note">{selected.filename} · SHA {selected.sha256?.slice(0,12)||"待校验"} · {selected.conv_channels||"?"}ch / {selected.num_blocks||"?"} blocks</p>:<p className="model-note">尚无可用模型，请先导入兼容的 Mortal .pth 权重。</p>}{message&&<p className="model-note">{message}</p>}</div>;
}

function Results({result,onNew,onCreated,initialExtend,onExtendOpened}:{result:RunResult;onNew:()=>void;onCreated:()=>void;initialExtend:boolean;onExtendOpened:()=>void}){
  const candidates=(result.candidates||result.summaries||[]).map(normalizeCandidate); const base=candidates[0]; const comparison=result.comparisons?.[0]; const winner=candidates.reduce((best,c)=>((c.avg_point??-Infinity)>(best?.avg_point??-Infinity)?c:best),base); const [open,setOpen]=useState<Record<string,boolean>>({value:true,outcome:true,win:true,defense:true,riichi:true,tenpai:false,call:false,yaku:false}); const [drawer,setDrawer]=useState<{candidate:Candidate;metric:string}|null>(null);
  const [extend,setExtend]=useState(false);
  useEffect(()=>{if(initialExtend){setExtend(true);onExtendOpened();}},[initialExtend,onExtendOpened]);
  const delta=comparison?.point_delta; const clear=delta?.ci95&&(delta.ci95[0]>0||delta.ci95[1]<0);
  const pointMetricsAvailable=result.metrics_version===2;
  const canExtend=result.schema_version===2&&result.metrics_version===2&&Boolean(result.run_id);
  ...
  return <div className="result-page"><section className="result-context"><button className="icon-button" aria-label="返回新分析" title="返回新分析" onClick={onNew}><RotateCcw size={17}/></button><div><span>{roundLabel(result.config?.round||result.resolved_context?.round||"E1")}</span><Hand source={resultHand} small/></div><div><span>第一摸</span><Tile value={resultFirstTsumo} small/></div><div><span>宝牌指示</span><Tile value={resultDora} small/></div><button className="extend-button" disabled={!canExtend} onClick={()=>setExtend(true)} title={canExtend?"向本分析追加相同配置的模拟":"仅完整的 schema v2 分析可以扩容"}><Plus size={15}/>增加局数</button><div className="context-facts"><b>{result.total_runs||result.runs} 局 / 候选</b><span>seed {result.seed} · {result.model?.label||result.model?.id||"Mortal"} · AMP</span></div></section>
    ...
    {pointMetricsAvailable?<section className="verdict"><div className="verdict-tile"><Tile value={winner?.discard||"1s"}/></div><div><span className="kicker">RECOMMENDED DISCARD</span><h2>推荐第一打：{winner&&candidateLabel(winner)}</h2><p>{comparison?<>相对 {candidateLabel(base)} 配对局收支 <strong>{signed(delta?.value)}</strong>，95% CI {ci(delta?.ci95)}。</>:"当前只有一个候选，可查看完整分布。"}</p></div><span className={`confidence ${clear?"clear":""}`}>{clear?"差异明确":"尚不明确"}</span></section>:<section className="verdict legacy-verdict"><div><span className="kicker">RETIRED METRICS</span><h2>旧记录需要重新模拟</h2><p>该记录不再生成推荐结论或局收支比较。</p></div></section>}
...
function History({runs,activeTasks,open,extend,refresh}:{runs:RunRecord[];activeTasks:ActiveTask[];open:(run:RunRecord)=>void;extend:(run:RunRecord)=>void;refresh:()=>void}){
  const remove=async(id:string)=>{if(!window.confirm("确定删除这条历史分析？此操作无法撤销。"))return;await fetch(`/api/runs/${id}`,{method:"DELETE"});refresh();};
  return <section className="surface history"><div className="surface-head"><div><span className="kicker">RUN ARCHIVE</span><h2>本机历史结果</h2></div><span className="quiet">{runs.length} 条记录</span></div><div className="history-head"><span>运行时间 / 候选</span><span>状态</span><span>版本</span><span>操作</span></div>{runs.map(run=>{const extending=activeTasks.some(task=>task.extension_of===run.run_id);const canExtend=run.status==="completed"&&run.result?.schema_version===2&&run.result?.metrics_version===2&&!extending;return <div className="history-row" key={run.run_id}><button className="history-open" onClick={()=>open(run)}><span><b>{run.request.discards?.map(requestCandidateLabel).join(" / ")}</b><small>{new Date(run.created_at).toLocaleString()} · {(run.result?.total_runs||run.result?.runs||run.request.runs||0).toLocaleString()} 局 · {run.result?.model?.label||run.request.model_id||"Mortal v4"}</small></span><span className={`run-badge ${extending?"live":""}`}>{extending?"扩容中":run.status}</span><code>schema v{run.result?.schema_version||1} / metrics v{run.result?.metrics_version||1}</code></button><div className="history-actions"><button className="icon-button" aria-label="增加局数" title={extending?"该分析正在扩容":canExtend?"增加局数":"仅新口径 metrics v2 记录可扩容"} disabled={!canExtend} onClick={()=>extend(run)}><Plus size={16}/></button><button className="icon-button" aria-label="删除" title="删除" onClick={()=>remove(run.run_id)}><X size={16}/></button></div></div>})}{!runs.length&&<div className="empty-state"><Archive size={30}/><b>还没有历史运行</b><span>完成一次分析后，结果会自动保存在本机。</span></div>}</section>;
}
function Diagnostics({cap,models,refreshModels}:{cap:any;models:ModelInfo[];refreshModels:()=>void}){const readyModels=models.filter(model=>model.ready);return <div className="diagnostics"><section className="surface"><span className="kicker">CUDA RUNTIME</span><h2>运行能力</h2>{[["GPU",cap?.gpu_name],["CUDA",cap?.cuda_version],["PyTorch",cap?.torch_version],["模型",readyModels.length?`${readyModels.length} 个可用`:"未导入"],["数据目录",cap?.data_dir]].map(([k,v])=><div className="diagnostic-row" key={k}><span>{k}</span><code>{v||"不可用"}</code></div>)}</section><section className="surface about"><span className="kicker">ABOUT</span><h2>MortalSim</h2><Brand/><p>本地运行的日麻第一打模拟与候选比较工具。正式推理使用 CUDA + PyTorch AMP，模拟核心由 Rust 实现。</p><p>应用默认仅监听 127.0.0.1，不上传手牌、seed、结果或日志。</p></section><section className="surface model-library"><div className="surface-head"><div><span className="kicker">LOCAL MODEL LIBRARY</span><h2>本机模型</h2></div><button className="icon-button" title="刷新模型库" aria-label="刷新模型库" onClick={refreshModels}><RotateCcw size={16}/></button></div>{models.map(model=><div className="model-row" key={model.id}><b>{model.label}</b><span>v{model.version} · {model.conv_channels}ch / {model.num_blocks} blocks · {model.ready?"可用":"不可用"}</span><code>{model.sha256?.slice(0,16)||"未校验"}</code></div>)}</section></div>}
