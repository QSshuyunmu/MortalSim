...
type Candidate = {
  discard: string; candidate?: string; first_riichi?: boolean; games: number; completed_games: number; errors: number; hanchan?: any; avg_point: number | null; avg_rank: number | null; point_ci95: number[] | null; rank_counts: number[];
...
function tileAssetPath(value:string):string {
  const display=publicTile(value);
  const n=display[0], suit=display[1];
  const honorFiles=["Ton","Nan","Shaa","Pei","Haku","Hatsu","Chun"];
  const suitFiles:Record<string,string>={m:"Man",p:"Pin",s:"Sou"};
  const file=suit==="z"?honorFiles[Number(n)-1]:`${suitFiles[suit]}${n==="0"?"5-Dora":n}`;
  return `${TILE_ASSET_ROOT}${file}${file==="Haku"?".svg":".webp"}`;
}

function drawTileImage(ctx:CanvasRenderingContext2D, value:string, x:number, y:number, w:number, h:number):Promise<void> {
  return new Promise((resolve)=>{
    ...
    img.onerror=()=>{
      ...
      ctx.fillStyle="#333";
      ctx.font=`${Math.max(10,Math.round(h/5))}px "Microsoft YaHei", sans-serif`;
      ctx.fillText(publicTile(value), x+2, y+h-4);
      resolve();
    };
    img.src=tileAssetPath(value);
  });
}

function pngText(ctx:CanvasRenderingContext2D, text:string, x:number, y:number, size:number, color="#111", bold=false) {
  ctx.fillStyle=color;
...
async function exportResultPng(
  ...
  const infoTop=190;
  const tileSize=44;
  const tileH=Math.round(tileSize*4/3);
  const tableTop=430;
  const rowH=58;
  const height=tableTop + Math.max(1,candidates.length)*rowH + 70;
  ...
  ctx.fillStyle="#0B5D50";
  ctx.fillRect(0,0,width,10);

  pngText(ctx,"MortalSim 第一打比较",40,66,34,"#0B5D50",true);
  pngText(ctx,`推荐第一打：${recommended?candidateLabel(recommended):"—"}`,40,108,22,"#111",true);

  // 初始设置
  pngText(ctx,"初始设置",40,158,20,"#333",true);
  const handTiles=parseTiles(config?.hand||"");
  const startX=40;
  for(let i=0;i<handTiles.length;i++){
    await drawTileImage(ctx,handTiles[i],startX+i*(tileSize+2),infoTop,tileSize,tileH);
  }
  pngText(ctx,"宝牌指示",40,infoTop+tileH+26,16,"#555",true);
  await drawTileImage(ctx,publicTile(config?.dora||"1z"),40,infoTop+tileH+32,tileSize,tileH);
  const kyotaku=Number(config?.kyotaku||0);
  const self=Number(config?.scores?.self||25000);
  const shimocha=Number(config?.scores?.shimocha||25000);
  const toimen=Number(config?.scores?.toimen||25000);
  const kamicha=100000-kyotaku*1000-self-shimocha-toimen;
  pngText(ctx,`${config?.round||"E1"}  ${config?.honba||0} 本场  ${kyotaku} 供托`,210,infoTop+tileH+44,17,"#333");
  pngText(ctx,`自家 ${self}    下家 ${shimocha}    对面 ${toimen}    上家 ${kamicha}`,210,infoTop+tileH+72,17,"#333");
  ...
  let cx=40;
  ctx.fillStyle="#0B5D50";
  ctx.fillRect(40,tableTop,width-80,36);
  pngText(ctx,"候选",cx+10,tableTop+24,15,"#fff",true); cx+=cols[0][1];
  for(let c=1;c<cols.length;c++){
    pngText(ctx,cols[c][0],cx+6,tableTop+24,14,"#fff",true); cx+=cols[c][1];
  }
  let y=tableTop+36;
  for(let i=0;i<candidates.length;i++){
    const c=candidates[i];
    const isRec=recommended&&candidateKey(c)===candidateKey(recommended);
    if(isRec){ ctx.fillStyle="#e6f4f0"; ctx.fillRect(40,y,width-80,rowH); }
    cx=40;
    const texts=[
      candidateLabel(c),
      c.value?.point?.value==null?"—":signed(c.value.point.value,0),
      c.value?.point?.ci95?ci(c.value.point.ci95):"样本不足",
      c.hanchan?.expected_rank?.value==null?"—":num(c.hanchan.expected_rank.value,2),
      c.hanchan?.rank_rates?.[0]?.rate==null?"—":pct(c.hanchan.rank_rates[0].rate),
      c.hanchan?.rank_rates?.[1]?.rate==null?"—":pct(c.hanchan.rank_rates[1].rate),
      c.hanchan?.rank_rates?.[2]?.rate==null?"—":pct(c.hanchan.rank_rates[2].rate),
      c.hanchan?.rank_rates?.[3]?.rate==null?"—":pct(c.hanchan.rank_rates[3].rate),
      c.hanchan?.dan_pt_ev?.houou_7?.value==null?"—":signed(c.hanchan.dan_pt_ev.houou_7.value,1),
      c.hanchan?.dan_pt_ev?.houou_8?.value==null?"—":signed(c.hanchan.dan_pt_ev.houou_8.value,1),
      c.hanchan?.dan_pt_ev?.houou_9?.value==null?"—":signed(c.hanchan.dan_pt_ev.houou_9.value,1),
      c.hanchan?.dan_pt_ev?.houou_10?.value==null?"—":signed(c.hanchan.dan_pt_ev.houou_10.value,1),
    ];
    for(let col=0;col<texts.length;col++){
      ctx.fillStyle=isRec?"#0B5D50":"#222";
      ctx.font=col===0?`bold 15px "Microsoft YaHei", sans-serif`:`14px "Microsoft YaHei", sans-serif`;
      ctx.fillText(String(texts[col]).slice(0,24),cx+8,y+rowH/2+5);
      cx+=cols[col][1];
    }
    y+=rowH;
  }

  pngText(ctx,"由 MortalSim 生成 · 半庄终局予想基于牌谱统计模型",40,height-24,13,"#888");
  const blob = await new Promise<Blob|null>((res)=>canvas.toBlob(res,"image/png"));
  if(!blob) return;
  const url=URL.createObjectURL(blob);
  const a=document.createElement("a");
  a.href=url;
  a.download=`mortalsim-${result.run_id||"result"}.png`;
...
export default function App() {
  ...
  return <div className="shell">
    ...
    <main><header><div><span className="kicker">LOCAL RIICHI SIMULATION</span><h1>{page==="run"?"早巡模拟器":page==="history"?"历史运行":"设置与诊断"}</h1></div><div className="header-status"><button className={`task-toggle ${tasks.length?"active":""}`} onClick={()=>setTaskDrawer(true)} aria-label="运行任务"><Layers3 size={15}/>任务 {tasks.length||""}</button><Activity size={15}/><span>数据仅保留在本机</span><code>v0.3.0-rc.1</code></div></header>
...
function Workbench({config,update,updateScore,candidates,start,cancel,busy,status,progress,cap,gpu,models,refreshModels}:any){
  ...
  return <>
    <section className="context-band"><div className="context-title"><span>局面</span><b>{roundLabel(config.round)} {config.target_seat !== undefined ? `· ${["东家","南家","西家","北家"][config.target_seat] || "自亲"}` : "· 自亲"} {config.x > 1 ? `· 第 ${config.x} 打` : ""}</b></div><div className="context-hand"><Hand source={mainHaipai}/><span className="draw-sep"/>{firstTsumo&&<Tile value={firstTsumo}/>}</div><div className="context-dora"><span>宝牌指示</span><Tile value={config.dora} small/></div></section>
...
function Results({result,onNew,onCreated,onRerun,initialExtend,onExtendOpened}:{result:RunResult;onNew:()=>void;onCreated:()=>void;onRerun:()=>void;initialExtend:boolean;onExtendOpened:()=>void}){
  const candidates=(result.candidates||result.summaries||[]).map(normalizeCandidate); const base=candidates[0]; const pointWinner=candidates.reduce((best,c)=>((c.avg_point??-Infinity)>(best?.avg_point??-Infinity)?c:best),base); const ptValue=(c:Candidate)=>c.hanchan?.dan_pt_ev?.houou_7?.value ?? Number.NEGATIVE_INFINITY; const ptWinner=candidates.reduce((best,c)=>ptValue(c)>ptValue(best)?c:best,base); const inconsistent=Boolean(pointWinner&&ptWinner&&candidateKey(pointWinner)!==candidateKey(ptWinner)); const winner=inconsistent?ptWinner:pointWinner; const [open,setOpen]=useState<Record<string,boolean>>({core:true,outcome:true,win:true,riichi:true,defense:false,yaku:false}); const [drawer,setDrawer]=useState<{candidate:Candidate;metric:string}|null>(null);
  ...
  const pointMetricsAvailable=(result.metrics_version??0)>=2;
  const canExtend=result.schema_version===3&&(result.metrics_version??0)>=2&&result.merge_state_version===2&&result.decision_contract==="stable_advantage_v2"&&Boolean(result.run_id);
  ...
  return <div className="result-page"><section className="result-context"><button className="icon-button" aria-label="返回新分析" title="返回新分析" onClick={onNew}><RotateCcw size={17}/></button><div><span>{roundLabel(result.config?.round||result.resolved_context?.round||"E1")}</span><Hand source={resultHand} small/></div><div><span>第一摸</span><Tile value={resultFirstTsumo} small/></div><div><span>宝牌指示</span><Tile value={resultDora} small/></div>{needsFormalRerun?<button className="extend-button" onClick={onRerun} title="复制局面与 seed，按当前正式 Lite 创建新记录"><RotateCcw size={15}/>重跑正式 Lite</button>:<button className="extend-button" disabled={!canExtend} onClick={()=>setExtend(true)} title={canExtend?"向本分析追加相同配置的模拟":"决策契约、运行时身份或精确合并状态不允许扩容"}><Plus size={15}/>增加局数</button>}<div className="context-facts"><b>{result.total_runs||result.runs} 局 / 候选</b><span>seed {result.seed} · {result.model?.label||result.model?.id||"Mortal"} · {result.decision_contract==="stable_advantage_v2"?"正式 Lite v2":"旧 AMP 语义"}</span></div><span className={`contract-badge ${needsFormalRerun?"legacy-contract":""}`}>{result.decision_contract||"legacy_amp_v1"}</span></section>
    ...
    {result.schema_version===2&&(result.metrics_version??0)>=2&&<div className="legacy">该记录使用旧决策语义，只读且不能扩容。可复制原局面、seed、模型和候选，按正式 Lite 创建新记录。</div>}
    {result.schema_version===3&&result.merge_state_version===2&&<div className="legacy">该记录使用旧版精确扩容状态（metrics v2 / merge_state v2），在新版中只读且不能追加；如需半庄终局预想等新指标，请按当前正式 Lite 重跑。</div>}{result.schema_version===3&&(result.merge_state_version??0)<2&&<div className="legacy">该早期 RC 记录缺少双响等终局所需的精确扩容状态，只读且不能追加；请按当前正式 Lite 重跑。</div>}
    {pointMetricsAvailable?<section className="verdict"><div className="verdict-tile"><Tile value={winner?.discard||"1s"}/></div><div><span className="kicker">RECOMMENDED DISCARD</span><h2>推荐第一打：{winner&&candidateLabel(winner)}</h2><p>{comparison&&verdictReference?<>{inconsistent?<>局收支最优为 <strong>{candidateLabel(pointWinner)}</strong>，但预想pt（鳳七段）最优为 <strong>{candidateLabel(ptWinner)}</strong>，倾向不一致；按预想pt推荐。</>:<>相对 {candidateLabel(verdictReference)} 配对局收支 <strong>{signed(delta?.value)}</strong>，95% CI {ci(delta?.ci95)}。</>}</>:"当前只有一个候选，可查看完整分布。"}</p></div><span className={`confidence ${!inconsistent&&clear?"clear":""}`}>{inconsistent?"倾向不一致，按pt推荐":clear?"明确优选":"尚不明确"}</span></section>:<section className="verdict legacy-verdict"><div><span className="kicker">RETIRED METRICS</span><h2>旧记录需要重新模拟</h2><p>该记录不再生成推荐结论或局收支比较。</p></div></section>}
    {result.warnings?.map(w=><div className="notice" key={w}>{w}</div>)}
    <section className="metric-workbench"><div className="metric-toolbar"><div><span className="kicker">FULL METRICS</span><h2>候选指标总表</h2></div><details className="export-menu"><summary><Download size={15}/>导出结果</summary><div role="menu"><a role="menuitem" href={`/api/runs/${result.run_id}/export?format=xlsx`}><b>Excel</b><span>当前总表中的全部统计指标</span></a><a role="menuitem" href={`/api/runs/${result.run_id}/export?format=json`}><b>JSON</b><span>完整结果协议，适合复现和二次分析</span></a><a role="menuitem" href={`/api/runs/${result.run_id}/export?format=html`}><b>HTML</b><span>无需应用即可打开的离线报告</span></a><a role="menuitem" href="#" onClick={(e)=>{e.preventDefault();exportResultPng(result,result.config||{},candidates,winner);}}><b>PNG</b><span>精简核心指标图，适合分享</span></a></div></details></div>
      ...
      <Group id="core" label="核心指标" open={open.core} toggle={()=>setOpen(o=>({...o,core:!o.core}))}><Row label="平均局收支（NAGA 口径）" values={candidates.map(c=>(result.metrics_version??0)>=2?signed(c.value.point.value):"不可用")}/><Row label="局收支 95% CI" values={candidates.map(c=>(result.metrics_version??0)>=2?ci(c.value.point.ci95):"不可用")}/><Row label="予想半荘終了時順位" values={candidates.map(c=>c.hanchan?num(c.hanchan.expected_rank?.value,3):"不可用")}/><Row label="順位 95% CI" values={candidates.map(c=>c.hanchan&&c.hanchan.expected_rank?.ci95?`${num(c.hanchan.expected_rank.ci95[0],3)} 至 ${num(c.hanchan.expected_rank.ci95[1],3)}`:"样本不足")}/>{[0,1,2,3].map(i=><Row key={i} label={`予想 ${i+1} 位率`} values={candidates.map(c=>c.hanchan?.rank_rates?.[i]?.rate==null?"不可用":pct(c.hanchan.rank_rates[i].rate))}/>)}{["houou_7","houou_8","houou_9","houou_10"].map(dan=><Row key={dan} label={`予想終了時${dan==="houou_7"?"鳳七段":dan==="houou_8"?"鳳八段":dan==="houou_9"?"鳳九段":"鳳十段"}段位pt収支`} values={candidates.map(c=>{const ev=c.hanchan?.dan_pt_ev?.[dan];return ev&&ev.value!=null?`${signed(ev.value)} (${ci(ev.ci95)})`:"不可用";})}/>)}</Group>
      <Group id="outcome" label="终局分布" open={open.outcome} toggle={()=>setOpen(o=>({...o,outcome:!o.outcome}))}>{OUTCOME_META.map(([id,label])=><ClickableRow key={id} label={label} candidates={candidates} metric={`outcome.${id}`} values={candidates.map(c=>rateText(c.outcome[id]))} open={setDrawer}/>)}</Group>
      <Group id="win" label="和牌构成与打点" open={open.win} toggle={()=>setOpen(o=>({...o,win:!o.win}))}><Row label="自摸 / 荣和" values={candidates.map(c=>`${pct(c.win.tsumo_share?.rate)} / ${pct(c.win.ron_share?.rate)}`)}/><Row label="立直和牌占比" values={candidates.map(c=>rateText(c.win.riichi_share))}/><Row label="默听和牌占比" values={candidates.map(c=>rateText(c.win.dama_share))}/><Row label="副露和牌占比" values={candidates.map(c=>rateText(c.win.open_share))}/><Row label="平均和牌点" values={candidates.map(c=>num(c.win.average_point))}/><Row label="平均和牌素点" values={candidates.map(c=>num(c.win.average_raw_point?.value))}/><Row label="平均翻数 / 符" values={candidates.map(c=>`${num(c.win.average_han?.value,2)} / ${num(c.win.average_fu?.value,1)}`)}/></Group>
      ...
      <Group id="defense" label="防守" open={open.defense} toggle={()=>setOpen(o=>({...o,defense:!o.defense}))}><Row label="平均放铳损失" negative values={candidates.map(c=>num(c.defense.average_deal_in_loss))}/><Row label="平均放铳巡目" values={candidates.map(c=>num(c.defense.average_deal_in_turn,2))}/></Group>
