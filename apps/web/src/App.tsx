...
function Results({result,onNew,onCreated,onRerun,initialExtend,onExtendOpened}:{result:RunResult;onNew:()=>void;onCreated:()=>void;onRerun:()=>void;initialExtend:boolean;onExtendOpened:()=>void}){
  ...
  const canExtend=result.schema_version===3&&result.metrics_version===2&&result.decision_contract==="stable_advantage_v2"&&Boolean(result.run_id);
  ...
  return <div className="result-page"><section className="result-context"><button className="icon-button" aria-label="返回新分析" title="返回新分析" onClick={onNew}><RotateCcw size={17}/></button><div><span>{roundLabel(result.config?.round||result.resolved_context?.round||"E1")}</span><Hand source={resultHand} small/></div><div><span>第一摸</span><Tile value={resultFirstTsumo} small/></div><div><span>宝牌指示</span><Tile value={resultDora} small/></div>{legacySchema?<button className="extend-button" onClick={onRerun} title="复制局面与 seed，按正式 Lite 创建新记录"><RotateCcw size={15}/>重跑正式 Lite</button>:<button className="extend-button" disabled={!canExtend} onClick={()=>setExtend(true)} title={canExtend?"向本分析追加相同配置的模拟":"决策契约或运行时身份不允许扩容"}><Plus size={15}/>增加局数</button>}<div className="context-facts"><b>{result.total_runs||result.runs} 局 / 候选</b><span>seed {result.seed} · {result.model?.label||result.model?.id||"Mortal"} · {result.decision_contract==="stable_advantage_v2"?"正式 Lite v2":"旧 AMP 语义"}</span></div><span className={`contract-badge ${legacySchema?"legacy-contract":""}`}>{result.decision_contract||"legacy_amp_v1"}</span></section>
...
function History({runs,activeTasks,open,extend,rerun,refresh}:{runs:RunRecord[];activeTasks:ActiveTask[];open:(run:RunRecord)=>void;extend:(run:RunRecord)=>void;rerun:(id:string)=>void;refresh:()=>void}){
  ...
  return <section className="surface history"><div className="surface-head"><div><span className="kicker">RUN ARCHIVE</span><h2>本机历史结果</h2></div><span className="quiet">{runs.length} 条记录</span></div><div className="history-head"><span>运行时间 / 候选</span><span>状态</span><span>版本</span><span>操作</span></div>{runs.map(run=>{const extending=activeTasks.some(task=>task.extension_of===run.run_id);const formal=run.result?.schema_version===3&&run.result?.decision_contract==="stable_advantage_v2";const canExtend=run.status==="completed"&&formal&&run.result?.metrics_version===2&&!extending;const canRerun=run.status==="completed"&&!formal;return <div className="history-row" key={run.run_id}><button className="history-open" onClick={()=>open(run)}><span><b>{run.request.discards?.map(requestCandidateLabel).join(" / ")}</b><small>{new Date(run.created_at).toLocaleString()} · {(run.result?.total_runs||run.result?.runs||run.request.runs||0).toLocaleString()} 局 · {run.result?.model?.label||run.request.model_id||"Mortal v4"}</small></span><span className={`run-badge ${extending?"live":""}`}>{extending?"扩容中":run.status}</span><code>{formal?"Formal Lite v2":`schema v${run.result?.schema_version||1} · 旧语义`}</code></button><div className="history-actions">{canRerun&&<button className="icon-button" aria-label="按正式 Lite 重跑" title="复制局面与 seed，创建正式 Lite 新记录" onClick={()=>rerun(run.run_id)}><RotateCcw size={16}/></button>}<button className="icon-button" aria-label="增加局数" title={extending?"该分析正在扩容":canExtend?"增加局数":"只有 schema v3 / stable_advantage_v2 记录可以扩容"} disabled={!canExtend} onClick={()=>extend(run)}><Plus size={16}/></button><button className="icon-button" aria-label="删除" title="删除" onClick={()=>remove(run.run_id)}><X size={16}/></button></div></div>})}{!runs.length&&<div className="empty-state"><Archive size={30}/><b>还没有历史运行</b><span>完成一次分析后，结果会自动保存在本机。</span></div>}</section>;
}
